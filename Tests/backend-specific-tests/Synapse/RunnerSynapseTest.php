...
class RunnerSynapseTest extends BaseRunnerTest
{
    ...
    public function testAbsWorkspaceOutputTablesAsFilesWithTags()
    {
        $this->clearBuckets();
        $componentData = [
            'id' => 'keboola.runner-workspace-abs-test',
        ...
        // but the file should exist
        $fileList = $this->client->listFiles((new ListFilesOptions())->setQuery(
            sprintf('tags:"componentId: keboola.runner-workspace-abs-test" AND tags:"configurationId: %s"', $configId)
        ));
        self::assertCount(1, $fileList);
        self::assertEquals('my_table.csv', $fileList[0]['name']);
        self::assertArraySubset(['foo', 'boo'], $fileList[0]['tags']);
    }
}
